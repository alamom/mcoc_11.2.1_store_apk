  ...
  implements Animatable
{
  ...
  private void applyFinishTranslation(float paramFloat, Ring paramRing)
  {
    ...
    float f2 = (float)(Math.floor(paramRing.getStartingRotation() / 0.8F) + 1.0D);
    float f1 = getMinProgressArc(paramRing);
    paramRing.setStartTrim(paramRing.getStartingStartTrim() + (paramRing.getStartingEndTrim() - f1 - paramRing.getStartingStartTrim()) * paramFloat);
    paramRing.setEndTrim(paramRing.getStartingEndTrim());
    paramRing.setRotation(paramRing.getStartingRotation() + (f2 - paramRing.getStartingRotation()) * paramFloat);
  }
  ...
  private int evaluateColorChange(float paramFloat, int paramInt1, int paramInt2)
  {
    ...
    return (int)(((paramInt2 >> 24 & 0xFF) - i) * paramFloat) + i << 24 | (int)(((paramInt2 >> 16 & 0xFF) - paramInt1) * paramFloat) + paramInt1 << 16 | (int)(((paramInt2 >> 8 & 0xFF) - j) * paramFloat) + j << 8 | (int)(((paramInt2 & 0xFF) - k) * paramFloat) + k;
  }
      ...
      public void applyTransformation(float paramAnonymousFloat, Transformation paramAnonymousTransformation)
      {
        ...
        for (;;)
        {
          ...
          if (paramAnonymousFloat > 0.5F)
          {
            ...
            localRing.setEndTrim(f3 + f4 * (0.8F - f1));
          }
  ...
  private static class Ring
  {
    ...
    private void drawTriangle(Canvas paramCanvas, float paramFloat1, float paramFloat2, Rect paramRect)
    {
      ...
      for (;;)
      {
        ...
        this.mArrow.offset(f1 - f3 * f2, f4);
        this.mArrow.close();
    ...
    public void draw(Canvas paramCanvas, Rect paramRect)
    {
      ...
      float f1 = (this.mEndTrim + this.mRotation) * 360.0F - f2;
...
/* Location:              C:\tools\androidhack\com.kabam.marvelbattle\classes.jar!\android\support\v4\widget\MaterialProgressDrawable.class
