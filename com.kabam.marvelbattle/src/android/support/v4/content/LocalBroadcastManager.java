      ...
      synchronized (this.mReceivers)
      {
        ...
        if ((paramIntent.getFlags() & 0x8) != 0)
        {
          ...
          if (i != 0)
          {
            ...
            Log.v("LocalBroadcastManager", "Resolving type " + str3 + " scheme " + str1 + " of intent " + paramIntent);
          }
        ...
        k = ((ReceiverRecord)localObject4).filter.match(str2, str3, str1, (Uri)localObject3, localSet, "LocalBroadcastManager");
        if (k >= 0)
        {
          if (i != 0)
          {
    ...
    synchronized (this.mReceivers)
    {
      ...
      for (int j = 0; j < localArrayList1.size(); j++)
      {
        ...
        for (int k = 0; k < localIntentFilter.countActions(); k++)
        {
          ...
          {
            int m;
            for (int i = 0; i < localArrayList2.size(); i = m + 1)
            {
              m = i;
...
/* Location:              C:\tools\androidhack\com.kabam.marvelbattle\classes.jar!\android\support\v4\content\LocalBroadcastManager.class
