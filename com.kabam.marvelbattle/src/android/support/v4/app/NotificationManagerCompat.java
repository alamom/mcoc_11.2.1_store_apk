...
public class NotificationManagerCompat
{
  ...
  public static Set<String> getEnabledListenerPackages(Context paramContext)
  {
    ...
    if ((str != null) && (!str.equals(sEnabledNotificationListeners)))
    {
      ...
      paramContext = new HashSet(arrayOfString.length);
      int j = arrayOfString.length;
      for (int i = 0; i < j; i++)
      {
        ??? = ComponentName.unflattenFromString(arrayOfString[i]);
        if (??? != null) {
          paramContext.add(((ComponentName)???).getPackageName());
        }
      }
    }
    synchronized (sEnabledNotificationListenersLock)
    {
      sEnabledNotificationListenerPackages = paramContext;
    ...
    private void updateListenerMap()
    {
      ...
      for (;;)
      {
        ...
        while (((Iterator)localObject2).hasNext())
        {
          ...
          if (!this.mRecordMap.containsKey(localObject1))
          {
            if (Log.isLoggable("NotifManCompat", 3)) {
              Log.d("NotifManCompat", "Adding listener record for " + localObject1);
            }
            this.mRecordMap.put(localObject1, new ListenerRecord((ComponentName)localObject1));
          }
        }
...
/* Location:              C:\tools\androidhack\com.kabam.marvelbattle\classes.jar!\android\support\v4\app\NotificationManagerCompat.class
