  ...
  extends LoaderManager
{
  ...
  public <D> Loader<D> restartLoader(int paramInt, Bundle paramBundle, LoaderManager.LoaderCallbacks<D> paramLoaderCallbacks)
  {
    ...
    LoaderInfo localLoaderInfo2 = (LoaderInfo)this.mLoaders.get(paramInt);
    if (DEBUG) {
      Log.v("LoaderManager", "restartLoader in " + this + ": args=" + paramBundle);
    }
    if (localLoaderInfo2 != null)
    {
      LoaderInfo localLoaderInfo1 = (LoaderInfo)this.mInactiveLoaders.get(paramInt);
      if (localLoaderInfo1 == null) {
        break label334;
      }
      if (!localLoaderInfo2.mHaveData) {
        break label177;
      }
      if (DEBUG) {
        Log.v("LoaderManager", "  Removing last inactive loader: " + localLoaderInfo2);
      }
      localLoaderInfo1.mDeliveredData = false;
      localLoaderInfo1.destroy();
      localLoaderInfo2.mLoader.abandon();
      this.mInactiveLoaders.put(paramInt, localLoaderInfo2);
    }
    for (;;)
    {
      for (paramBundle = createAndInstallLoader(paramInt, paramBundle, paramLoaderCallbacks).mLoader;; paramBundle = localLoaderInfo2.mPendingLoader.mLoader)
      {
        return paramBundle;
        label177:
        if (!localLoaderInfo2.mStarted)
        {
          if (DEBUG) {
        ...
        localLoaderInfo2.cancel();
        if (localLoaderInfo2.mPendingLoader != null)
        {
          if (DEBUG) {
            Log.v("LoaderManager", "  Removing pending loader: " + localLoaderInfo2.mPendingLoader);
          }
          localLoaderInfo2.mPendingLoader.destroy();
          localLoaderInfo2.mPendingLoader = null;
        }
        if (DEBUG) {
          Log.v("LoaderManager", "  Enqueuing as new pending loader");
        }
        localLoaderInfo2.mPendingLoader = createLoader(paramInt, paramBundle, paramLoaderCallbacks);
      }
      label334:
      if (DEBUG) {
        Log.v("LoaderManager", "  Making last loader inactive: " + localLoaderInfo2);
      }
      localLoaderInfo2.mLoader.abandon();
      this.mInactiveLoaders.put(paramInt, localLoaderInfo2);
    }
...
/* Location:              C:\tools\androidhack\com.kabam.marvelbattle\classes.jar!\android\support\v4\app\LoaderManagerImpl.class
