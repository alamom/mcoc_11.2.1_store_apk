  ...
  implements SafeParcelable, TurnBasedMatch
{
  ...
  static boolean a(TurnBasedMatch paramTurnBasedMatch, Object paramObject)
  {
    ...
    for (;;)
    {
      ...
      if (paramTurnBasedMatch != paramObject)
      {
        ...
        {
          bool1 = bool2;
          if (n.equal(Boolean.valueOf(((TurnBasedMatch)paramObject).isLocallyModified()), Boolean.valueOf(paramTurnBasedMatch.isLocallyModified()))) {}
        }
        else
        {
          bool1 = false;
  ...
  static String b(TurnBasedMatch paramTurnBasedMatch, String paramString)
  {
    ...
    if (i < j)
    {
      paramTurnBasedMatch = (Participant)localArrayList.get(i);
      Player localPlayer = paramTurnBasedMatch.getPlayer();
      if ((localPlayer == null) || (!localPlayer.getPlayerId().equals(paramString))) {}
    }
    for (paramTurnBasedMatch = paramTurnBasedMatch.getParticipantId();; paramTurnBasedMatch = null)
    {
      return paramTurnBasedMatch;
      i++;
      break;
    }
  }
  ...
  static ArrayList<String> c(TurnBasedMatch paramTurnBasedMatch)
  {
    ArrayList localArrayList = paramTurnBasedMatch.getParticipants();
    int j = localArrayList.size();
    paramTurnBasedMatch = new ArrayList(j);
    for (int i = 0; i < j; i++) {
      paramTurnBasedMatch.add(((Participant)localArrayList.get(i)).getParticipantId());
    }
    return paramTurnBasedMatch;
  }
  ...
  {
    if (this.acs == null) {}
    for (int i = 0;; i = this.acs.getInt("max_automatch_players")) {
      return i;
    }
  }
...
/* Location:              C:\tools\androidhack\com.kabam.marvelbattle\classes.jar!\com\google\android\gms\games\multiplayer\turnbased\TurnBasedMatchEntity.class
